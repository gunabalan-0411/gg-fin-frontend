...
import { Mic, MicOff, CheckCircle, Send, ChevronRight, Clock, Wifi, Zap, ArrowUpToLine, Search, X, Play, Pause, Strikethrough } from "lucide-react";
...
export default function VoicePage() {
  ...
  const scrollToRow = (customerId: number) => {
    setTimeout(() => {
      ...
      if (row && container) {
        container.scrollTo({ top: row.offsetTop - 33, behavior: "smooth" });
      }
    }, 50);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div ref={searchRef} className="relative flex-1 min-w-[200px]">
          ...
          {searchResults.length > 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 z-50 rounded-xl border border-border bg-card shadow-xl overflow-hidden max-h-72 overflow-y-auto">
              {searchResults.map((r) => (
                <div
                  key={r.customer_id}
                  className="flex items-center gap-2 px-3 py-2 hover:bg-secondary/60 border-b border-border/40 last:border-0"
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    />
                    <button
                      onClick={() => applySearchAmount(r.customer_id)}
                      disabled={!searchAmounts[r.customer_id]}
          ...
          {searchQuery.trim().length > 0 && searchResults.length === 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 z-50 rounded-xl border border-border bg-card shadow-xl px-3 py-4 text-center text-xs text-muted-foreground">
              No customers found
            </div>
          )}
        </div>
      ...
      {/* Two-column layout: 60% queue+mic | 40% UPI+table */}
      <div className="grid gap-6" style={{ gridTemplateColumns: "3fr 2fr" }}>

        {/* LEFT — top: [queue 60% | UPI 40%], bottom: mic */}
        <div className="flex flex-col gap-4">

          {/* TOP ROW — Queue + UPI side by side */}
          ...
          <div className="glass-card overflow-hidden p-0">
              ...
              {queue.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">
                    {queueDone ? "All done" : `${queueIndex + 1} / ${queue.length}`}
                  </span>
                  {/* Progress dots */}
            ...
            {queue.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 gap-2 text-muted-foreground/50">
                <div className="flex gap-1">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="w-8 h-10 rounded-lg border-2 border-dashed border-border/40" style={{ opacity: 1 - i * 0.25 }} />
                  ))}
                </div>
              ...
              <div className="overflow-y-auto max-h-[300px] px-3 py-3 space-y-2">
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between px-3 pt-2.5 pb-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {isActive && (
                            <span className="flex-shrink-0 text-[10px] font-bold text-primary bg-primary/20 px-1.5 py-0.5 rounded tracking-wide">
                              NOW
                            </span>
                          )}
                          {!isDone && !isActive && (
                          ...
                          <span className={`text-sm font-semibold truncate ${isDone ? "text-muted-foreground line-through" : "text-foreground"}`}>
                            {entry.spoken_name}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                          ...
                          {entry.customer_id && (
                            <button
                              onClick={() => {
                                setPinnedCustomerId((prev) => prev === entry.customer_id ? null : entry.customer_id!);
                                scrollToRow(entry.customer_id!);
                              }}
                              title="Bring to top of table"
                              className={`flex-shrink-0 p-0.5 rounded transition-colors ${
                                pinnedCustomerId === entry.customer_id
                      ...
                      {entry.customer_name && (
                        <div className="px-3 pb-1.5">
                          <span className="text-xs text-muted-foreground">
                            → {entry.customer_name}{" "}
                            <span className="text-primary/70">#{entry.customer_id}</span>
                          </span>
                        </div>
                      )}
                        ...
                        <div className="px-3 pb-3 pt-1 border-t border-border/30 mt-1">
                          {entry.matched ? (
                            <div className="flex items-center gap-2 py-1">
                              ...
                              <span className={`text-xs ${queueDisplayMode === "online" ? "text-blue-400" : "text-emerald-400"}`}>
                            ...
                            <div className="space-y-1.5">
                              ...
                              {entry.alternatives && entry.alternatives.length > 0 ? (
                                entry.alternatives.map((alt) => (
                                  ...
                                  >
                                    ...
                                    <span className={`text-xs font-semibold flex-shrink-0 ${alt.score >= 80 ? "text-emerald-400" : alt.score >= 60 ? "text-amber-400" : "text-muted-foreground"}`}>
                              ...
                              <button
                                ...
                                className="w-full text-xs text-muted-foreground hover:text-foreground py-1.5 transition-colors border-t border-border/30 mt-1 pt-2"
                              >
                                Skip this entry →
                              </button>
                            </div>
                ...
                {queueDone && (
                  <div className={`flex items-center justify-center gap-2 py-2.5 text-xs font-semibold rounded-xl border ${
          ...
          <div className="glass-card overflow-hidden p-0 flex flex-col">
            <div className="flex items-center justify-between px-3 py-2 border-b border-border flex-shrink-0">
              ...
              <button
                onClick={applyMappedUpiTransactions}
                className="flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded bg-primary/15 text-primary border border-primary/30 hover:bg-primary/25 transition-colors"
              >
                      ...
                      return (
                        <tr key={txn.id} className={`border-b border-border/40 transition-colors ${isStruck ? "opacity-50 bg-secondary/30" : "hover:bg-secondary/20"}`}>
                          ...
                          <td className="px-2 py-1.5 w-16 text-right flex-shrink-0">
                            <span className={`text-xs font-bold whitespace-nowrap ${isStruck ? "line-through text-muted-foreground" : "text-emerald-400"}`}>
                              ₹{Number(txn.amount).toLocaleString("en-IN")}
                            </span>
                          </td>
                          {/* Sender + customer + VPA */}
                          <td className="px-2 py-1.5 min-w-0">
                            <p className={`text-xs font-medium truncate leading-tight ${isStruck ? "line-through text-muted-foreground" : "text-foreground"}`}>
                              {txn.sender_name || "—"}
                            </p>
                            {isMapped ? (
                              <p className={`text-[10px] truncate leading-tight flex items-center gap-0.5 ${isStruck ? "line-through text-muted-foreground/50" : "text-emerald-400"}`}>
                                <CheckCircle className="h-2.5 w-2.5 flex-shrink-0 shrink-0" />
                            ...
                            {txn.sender_vpa && (
                              <p className="text-[9px] text-muted-foreground/40 truncate leading-tight font-mono">
                                {txn.sender_vpa}
                              </p>
                            )}
                          </td>
                          ...
                          <td className="px-1 py-1.5 w-7 text-center flex-shrink-0">
                            <button
                              ...
                              title={isStruck ? "Unstrike" : "Mark as done"}
                              className={`p-0.5 rounded transition-colors ${isStruck ? "text-muted-foreground/60 hover:text-foreground" : "text-muted-foreground/30 hover:text-muted-foreground"}`}
                            >
          ...
          {/* BOTTOM — Single mic card with mode toggle */}
          <div className="glass-card py-5 px-4">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex items-center gap-4">
                {/* Play/Pause — only shown when queue is active */}
                  ...
                  <button
                    ...
                    className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all ${
                      queuePaused
                        ...
                        : "border-muted-foreground/30 bg-secondary hover:bg-secondary/80 text-muted-foreground hover:text-foreground"
                    }`}
                  >
              ...
              >
                ...
                {recording && (
                  <span className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-card border border-red-500 flex items-center justify-center text-[10px] font-bold text-red-400">
                    {countdown}
                  </span>
                )}
              </button>
              </div>{/* end mic + play/pause row */}
              ...
              {loading ? (
                <div className="flex flex-col items-center gap-1 w-full">
                  ...
                  <button
                    ...
                    className="text-[10px] text-red-400 hover:text-red-300 underline underline-offset-2 mt-0.5"
                  >
                    Discard
                  </button>
                </div>
                ...
                <p className="text-[11px] text-muted-foreground text-center">
                  say name + amount <span className="text-primary/70">→ amount entry</span>
                </p>
              ) : (
                <p className="text-[11px] text-blue-400/70 text-center">
                  say name + amount <span className="text-blue-400">→ amount + online check</span>
                </p>
              )}
        ...
        <div className="space-y-3">
                          ...
                          <tr key="pinned-header">
                            <td colSpan={5} className="px-3 py-1 bg-amber-500/10 border-b border-amber-500/20">
                              <span className="text-[10px] font-semibold text-amber-400 uppercase tracking-wider flex items-center gap-1">
                                <ArrowUpToLine className="h-3 w-3" /> Pinned
                              </span>
                            </td>
                          </tr>
                          <tr
                            key={`pinned-${r.customer_id}`}
                            ref={(el) => { if (el) rowRefs.current.set(r.customer_id, el); else rowRefs.current.delete(r.customer_id); }}
                            ...
                            <td className="px-3 py-2">
                              <div className="flex flex-col gap-0.5">
                                <button type="button"
                                  ...
                                  className={`text-xs font-semibold px-2 py-0.5 rounded-full transition-colors ${r.payment_mode === "ONLINE" ? "bg-blue-500/20 text-blue-400 border border-blue-500/40" : "bg-emerald-500/20 text-emerald-400 border border-emerald-500/40"}`}
                                >{r.payment_mode}</button>
                                <button type="button"
                                  onClick={() => setRows((prev) => { const next = [...prev]; next[r.rowIdx] = { ...next[r.rowIdx], is_paid: !next[r.rowIdx].is_paid }; return next; })}
                                  className={`text-[10px] font-medium px-1.5 py-0.5 rounded transition-colors ${r.is_paid ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}
                                >{r.is_paid ? "Paid" : "Unpaid"}</button>
                              </div>
                            </td>
                        ...
                        {group.rows.filter((r) => r.customer_id !== pinnedCustomerId).map((r) => {
                          ...
                          return (
                            <tr
                              key={r.customer_id}
                              ref={(el) => {
                                if (el) rowRefs.current.set(r.customer_id, el);
                                else rowRefs.current.delete(r.customer_id);
                              }}
                              className={`border-b border-border/50 transition-colors ${
                                isHighlighted
                                  ? activeQueueModeRef.current === "online"
                                    ? "bg-blue-500/10 ring-1 ring-inset ring-blue-500/40"
                                    : "bg-primary/10 ring-1 ring-inset ring-primary/40"
                                  : "hover:bg-secondary/20"
                              }`}
                            >
                              <td className={`px-3 py-2 text-xs font-medium ${isHighlighted ? (activeQueueModeRef.current === "online" ? "text-blue-400" : "text-primary") : "text-muted-foreground"}`}>
                                {r.customer_id}
                              </td>
                              <td className={`px-3 py-2 font-medium truncate max-w-[120px] ${isHighlighted ? (activeQueueModeRef.current === "online" ? "text-blue-400" : "text-primary") : "text-foreground"}`}>
                                {r.customer_name}
                              </td>
                              <td className="px-3 py-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setRows((prev) => {
                                      const next = [...prev];
                                      next[r.rowIdx] = { ...next[r.rowIdx], amount: e.target.value };
                                      return next;
                                    })
                                  }
                                  className={`w-20 h-7 rounded border px-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring ${
                                    isHighlighted ? "border-primary/60 bg-primary/5" : "border-border bg-secondary"
                                  }`}
                                />
                              </td>
                              <td className="px-3 py-2">
                                <div className="flex flex-col gap-0.5">
                                  <button
                                    ...
                                    onClick={() =>
                                      setRows((prev) => {
                                        const next = [...prev];
                                        const newMode = next[r.rowIdx].payment_mode === "CASH" ? "ONLINE" : "CASH";
                                        next[r.rowIdx] = {
                                          ...next[r.rowIdx],
                                          payment_mode: newMode,
                                          is_paid: newMode === "CASH",
                                        };
                                        return next;
                                      })
                                    }
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      setRows((prev) => {
                                        const next = [...prev];
                                        next[r.rowIdx] = { ...next[r.rowIdx], is_paid: !next[r.rowIdx].is_paid };
                                        return next;
                                      })
                                    }
          ...
          <Button onClick={handleSubmitAll} disabled={submitting || isTableLoading} className="w-full">
            ...
            {submitting
              ? "Saving..."
              : `Submit All (${rows.filter((r) => Number(r.amount) > 0).length} with amount)`}
          </Button>
        </div>
      </div>
    </div>
